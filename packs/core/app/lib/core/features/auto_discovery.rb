      ...
      class << self
        ...
        private
          ...
          def derive_feature(pack_module_name)
            ...
            # RibCheckWorkflow -> :rib_requests (pluralized from rib_check -> rib_request)
            # SystemUsers -> :users
            ...
            name = pack_module_name
              ...
              .gsub(/^System/, "")
              .gsub(/^Admin/, "")
            feature = name.underscore

            # Special case for workflow packs: rib_check -> rib_requests
            feature = "#{feature.singularize}_requests" if pack_module_name.end_with?("Workflow")
          ...
          def discover_tools(pack_lib_path, pack_module_name)
            ...
            Dir[File.join(pack_lib_path, "tools", "*.rb")].each do |file|
              ...
              klass = class_name.safe_constantize
              # Accept both Core::Tools::Base and Core::Workflow::Base (which inherits from Tools::Base)
