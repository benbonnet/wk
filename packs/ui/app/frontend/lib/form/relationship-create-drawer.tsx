...
import { useServices, useTranslate } from "../provider";
import { DynamicRenderer } from "../renderer";
import type { UISchema } from "../types";
import { FormContext } from "@ui/adapters/layouts/form";
...
export const RelationshipCreateDrawer: FC<RelationshipCreateDrawerProps> = ({
  ...
  const getValue = useCallback((fieldName: string) => values[fieldName], [values]);
  const getError = useCallback((fieldName: string) => errors[fieldName], [errors]);
  const isTouched = useCallback((fieldName: string) => touched[fieldName] || false, [touched]);
