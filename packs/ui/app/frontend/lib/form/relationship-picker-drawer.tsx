...
import { RelationshipCreateDrawer } from "./relationship-create-drawer";
...
export const RelationshipPickerDrawer: FC<RelationshipPickerDrawerProps> = ({
  ...
  const [createdItems, setCreatedItems] = useState<Map<number, AttributePayload>>(new Map());
                    ...
                    >
                      ...
                      {columns.map((col) => (
                        <TableCell key={col.name}>
                          {String((item.data as Record<string, unknown>)?.[col.name] ?? "-")}
                        </TableCell>
                      ))}
                    </TableRow>
                  ))}
