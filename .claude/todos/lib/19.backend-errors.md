...
Based on existing schemas (Contact, RibRequest, Address, Email), here are concrete integration tests.

### Backend Integration Tests

**File:** `packs/contacts_service/spec/requests/contacts_validation_spec.rb`
...
RSpec.describe "Contact Validation Errors", type: :request do
  ...
  describe "POST /v1/contacts-service/contacts" do
    context "with missing required fields" do
      it "returns Rails-style errors for first_name and last_name" do
        post "/v1/contacts-service/contacts", params: {
          company: "Acme Corp"
        }
    ...
    context "with invalid enum value" do
      it "returns error for gender field" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["gender"]).to include("is not included in the list")
      end
    end

    context "with invalid date format" do
      it "returns error for date_of_birth field" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["date_of_birth"]).to include("is invalid")
      end
    end

    context "with invalid tags type" do
      it "returns error when tags is not an array" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
          tags: "not-an-array"
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["tags"]).to include("must be an array")
      end
    end

    context "with nested address errors" do
      it "returns structured errors for addresses_attributes" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
          addresses_attributes: [
            { label: "Home" },  # missing address_line_1 and city
            { address_line_1: "123 Main St", city: "NYC" }  # valid
          ]
        }
        ...
        body = JSON.parse(response.body)
        addresses_errors = body["details"]["addresses_attributes"]

        expect(addresses_errors).to be_an(Array)
        expect(addresses_errors[0]["address_line_1"]).to include("can't be blank")
        expect(addresses_errors[0]["city"]).to include("can't be blank")
        expect(addresses_errors[1]).to eq({})  # second address is valid
      end
    end

    context "with nested email errors" do
      it "returns structured errors for invalid email format" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
          emails_attributes: [
        ...
        body = JSON.parse(response.body)
        emails_errors = body["details"]["emails_attributes"]

        expect(emails_errors).to be_an(Array)
        expect(emails_errors[0]["address"]).to include("is invalid")
      end
    end

    context "with spouse (has_one) errors" do
      it "returns structured errors for spouse_attributes" do
        post "/v1/contacts-service/contacts", params: {
          first_name: "John",
          last_name: "Doe",
          spouse_attributes: {
            company: "Acme"  # missing first_name, last_name
          }
        }
        ...
        body = JSON.parse(response.body)
        spouse_errors = body["details"]["spouse_attributes"]

        expect(spouse_errors).to be_a(Hash)
        expect(spouse_errors["first_name"]).to include("can't be blank")
        expect(spouse_errors["last_name"]).to include("can't be blank")
      end
    end
  end
end
...
**File:** `packs/rib_check_service/spec/requests/rib_requests_validation_spec.rb`
...
RSpec.describe "RIB Request Validation Errors", type: :request do
  let(:user) { create(:user) }
  let(:workspace) { create(:workspace) }
  ...
  describe "POST /v1/rib-check-service/rib-requests" do
    context "with missing required enum fields" do
      it "returns errors for request_type and status" do
        post "/v1/rib-check-service/rib-requests", params: {
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["request_type"]).to include("can't be blank")
        expect(body["details"]["status"]).to include("can't be blank")
      end
    end

    context "with invalid enum values" do
      it "returns error for invalid request_type" do
        post "/v1/rib-check-service/rib-requests", params: {
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["request_type"]).to include("is not included in the list")
      end
      ...
      it "returns error for invalid status" do
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["status"]).to include("is not included in the list")
      end
    end

    context "with invalid datetime format" do
      it "returns error for end_at field" do
        post "/v1/rib-check-service/rib-requests", params: {
          request_type: "individual",
          status: "draft",
        ...
        body = JSON.parse(response.body)
        expect(body["details"]["end_at"]).to include("is invalid")
      end
    end

    context "with invalid boolean fields" do
      it "returns error for non-boolean notify_via_email" do
        post "/v1/rib-check-service/rib-requests", params: {
          request_type: "individual",
          status: "draft",
          notify_via_email: "yes"  # should be true/false
        }

        expect(response).to have_http_status(:unprocessable_content)

        body = JSON.parse(response.body)
        expect(body["details"]["notify_via_email"]).to include("must be true or false")
      end
    end

    context "with valid data" do
      it "creates the record successfully" do
        post "/v1/rib-check-service/rib-requests", params: {
          request_type: "individual",
          status: "draft",
          comment: "Test request",
...
**File:** `packs/ui/app/frontend/adapters/custom/form/__tests__/form-validation-errors.test.tsx`

```typescript
import { render, screen, waitFor } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { TestWrapper, mockFetch } from "@/test-utils";
import { Form } from "../form";

describe("Form validation errors", () => {
  describe("flat field errors", () => {
    it("displays errors on fields from API response", async () => {
      ...
      render(
        <TestWrapper>
          <Form
            ...
            endpoint="/v1/contacts-service/contacts"
          />
        </TestWrapper>
      );
  ...
  describe("nested array errors (has_many)", () => {
    it("displays errors on nested address fields", async () => {
      mockFetch.mockRejectedValueOnce({
        response: {
          status: 422,
          data: {
            ...
            details: {
              first_name: [],
              addresses_attributes: [
                { address_line_1: ["can't be blank"], city: ["can't be blank"] },
                {},
              ],
            },
          },
      ...
      render(
        <TestWrapper>
          <Form
            schema={contactSchemaWithAddresses}
            endpoint="/v1/contacts-service/contacts"
            initialValues={{
              first_name: "John",
              last_name: "Doe",
              addresses_attributes: [{ label: "Home" }, { address_line_1: "123 Main", city: "NYC" }],
            }}
          />
        </TestWrapper>
      );
      ...
      await waitFor(() => {
        // First address should show errors
        const addressForms = screen.getAllByTestId("nested-form-addresses");
        expect(addressForms[0]).toHaveTextContent("can't be blank");
      });
    });
  });

  describe("nested object errors (has_one)", () => {
    it("displays errors on spouse fields", async () => {
      mockFetch.mockRejectedValueOnce({
        response: {
          status: 422,
          data: {
      ...
      render(
        <TestWrapper>
          <Form
            schema={contactSchemaWithSpouse}
            endpoint="/v1/contacts-service/contacts"
            initialValues={{
              first_name: "John",
              last_name: "Doe",
              spouse_attributes: { company: "Acme" },
            }}
          />
        </TestWrapper>
      );
      ...
      await waitFor(() => {
        const spouseSection = screen.getByTestId("nested-form-spouse");
        expect(spouseSection).toHaveTextContent("can't be blank");
      });
    });
  });

  describe("enum validation", () => {
    it("displays error for invalid enum value", async () => {
      mockFetch.mockRejectedValueOnce({
        response: {
          status: 422,
          data: {
      ...
      await waitFor(() => {
        expect(screen.getByText("is not included in the list")).toBeInTheDocument();
      });
    });
  });
});
...
### Schema-Specific Validation Scenarios

| Schema     | Field            | Validation Type   | Expected Error Message         |
| ---------- | ---------------- | ----------------- | ------------------------------ |
| Contact    | first_name       | required          | `can't be blank`               |
| Contact    | last_name        | required          | `can't be blank`               |
| Contact    | gender           | enum              | `is not included in the list`  |
| Contact    | date_of_birth    | format: date      | `is invalid`                   |
| Contact    | tags             | type: array       | `must be an array`             |
| Address    | address_line_1   | required          | `can't be blank`               |
| Address    | city             | required          | `can't be blank`               |
| Email      | address          | format: email     | `is invalid`                   |
| RibRequest | request_type     | required + enum   | `can't be blank` / `not in...` |
| RibRequest | status           | required + enum   | `can't be blank` / `not in...` |
| RibRequest | end_at           | format: date-time | `is invalid`                   |
| RibRequest | notify_via_email | type: boolean     | `must be true or false`        |

### Expected API Response Structure

**Contact with nested address errors:**
...
```json
{
  ...
  "details": {
    ...
    "emails_attributes": [{ "address": ["is invalid"] }],
    "spouse_attributes": {
      "first_name": ["can't be blank"]
    }
  }
...
```json
{
  ...
  "details": {
    ...
    "end_at": ["is invalid"],
    "notify_via_email": ["must be true or false"]
  }
}
```
