...
│  - Passes relationships to compute_custom_attributes        │
└─────────────────────────────────────────────────────────────┘
                              │
              ┌───────────────┼───────────────┐
              ▼               ▼               ▼
...
module ContactsService
  class ContactSerializer < Core::Serializers::BaseItemSerializer
    ...
    private
      ...
      def compute_custom_attributes(item, relationships)
        {
...
Relationships are loaded **once** in `load_relationships` and passed as context to `compute_custom_attributes`:
...
def to_h
  ...
  computed = compute_custom_attributes(@object, relationships)
...
class ContactSerializer < BaseItemSerializer
  ...
  def self.computed_fields
    # Introspect compute_custom_attributes or maintain explicit list
  ...
  module Serializers
    class BaseItemSerializer
      ...
      private
        ...
        def compute_custom_attributes(_item, _relationships)
          {}
        end
    end
  end
...
module RibCheckService
  class RibRequestSerializer < Core::Serializers::BaseItemSerializer
    ...
    private
      ...
      def compute_custom_attributes(_item, relationships)
        {
