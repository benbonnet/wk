...
- [ ] Add `frontend_route` DSL to `Ui::Views::BaseView`
- [ ] Add `routable?` class method
- [ ] Add `frontend_routes` to `Core::Features::Registry`
- [ ] Create `Api::V1::RoutesController#index`
- [ ] Add route `resources :routes, only: [:index]` to `config/routes.rb`
- [ ] Add `frontend_route "/rib-checks"` to `RibCheckWorkflow::Views::Index`
- [ ] Add `frontend_route "/contacts"` to `ContactsService::Views::Index`
- [ ] Add `frontend_route "/contacts/:id"` to `ContactsService::Views::Show`
- [ ] Write request spec for `/api/v1/routes`

## Frontend Checklist

- [ ] Create `@ui/hooks/use-routes.ts`
- [ ] Create `@ui/lib/ui-renderer/view-page.tsx`
- [ ] Update `main.tsx` with dynamic routing
- [ ] Remove hardcoded ActivitiesIndex route
- [ ] Test `/rib-checks`
- [ ] Test `/contacts`
- [ ] Test `/contacts/:id`
...
**File:** `app/controllers/api/v1/routes_controller.rb`
